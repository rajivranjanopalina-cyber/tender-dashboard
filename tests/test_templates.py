...
FAKE_BLOB_URL = "https://blob.vercel-storage.com/templates/abc123_proposal.docx"
...
def test_upload_template_docx(client, tmp_path, monkeypatch):
    ...
    with patch("backend.routers.templates.upload_blob", return_value=FAKE_BLOB_URL):
        resp = client.post(
            "/api/templates",
...
def test_update_template_metadata(client, tmp_path, monkeypatch, db_session):
    ...
    t = models.Template(
        ...
        blob_url=FAKE_BLOB_URL, file_type="docx", sha256="abc", is_default=False,
    )
    db_session.add(t)
    db_session.commit()
...
def test_delete_template_no_active_proposals(client, tmp_path, monkeypatch, db_session):
    from backend import models
    t = models.Template(
        name="T", original_filename="t.docx", blob_url=FAKE_BLOB_URL,
        file_type="docx", sha256="abc", is_default=False,
    )
    db_session.add(t)
    db_session.commit()
    with patch("backend.routers.templates.delete_blob", return_value=None):
...
def test_delete_template_with_active_proposal_blocked(client, db_session, tmp_path):
    ...
    proposal = models.Proposal(
        tender_id=tender.id, template_id=t.id,
